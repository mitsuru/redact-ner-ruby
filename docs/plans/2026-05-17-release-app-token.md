...
5. In "Create branch, commit, open Release PR", **remove** the inline `git config user.name "github-actions[bot]"` / `git config user.email ...` lines (now set by the dedicated identity step). Keep `git switch -c "release/$tag"`, `git add`, `git commit -m "Release $tag"`, `git push -u origin "release/$tag"`, `gh label create release ... || true`, `gh pr create --base main --head "release/$tag" --label release ...`.
